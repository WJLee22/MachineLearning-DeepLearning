# 수치미분 구현(1차 버전): 입력변수가 x 하나인 1차 함수 f(x) = x 에 대한  간단한 수치미분 구현. 
...
# 즉 수치미분 코드는, 입력으로 들어오는 x값이 아주미세하게 변할 때, 과연 함수 f(x)는 얼마나 변하는지를 수치적으로 계산해서 반환해주는 함수임.

# 이 후에 다변수 함수에 대한 수치미분을 구현할 것임.